...
angular.module('CornerCouch', ['ng']).
    factory('cornercouch', ['$http', function ($http) {
        ...
        CouchDB.prototype.queryViewPost = function (viewURL, qparams) {
            var config = {
                method: this.method,
                url: this.uri + viewURL
            };
        ...
        CouchDB.prototype.update = function (design, update, doc_id, qparams) {
            return this.queryView(
                "/_design/" + encodeURIComponent(design) +
                    "/_update/" + encodeURIComponent(update) +
                    "/" + encodeURIComponent(doc_id),
                qparams
            );
        };

        CouchDB.prototype.show = function (design, show, doc_id, qparams) {
            $http.defaults.withCredentials = true;
            return this.queryViewPost(
                "/_design/" + encodeURIComponent(design) +
                    "/_show/" + encodeURIComponent(show) +
                    "/" + encodeURIComponent(doc_id),
                qparams
            );
        };
